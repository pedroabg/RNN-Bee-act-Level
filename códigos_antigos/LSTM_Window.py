...
def create_dataset(dataset, look_back=1):
 ...
 #exemplo len(dataset) = 10, look_back = 1, entao range = 8, loop de 0 a 7
	for i in range(len(dataset)-look_back):
		...
		#print(a)
		a =  numpy.concatenate((dataset[i + look_back][0:6] , a))
		dataX.append(a)
...
# load the dataset
dataframe = pandas.read_csv('abelha-original-formatado.csv', engine='python')
...
model = Sequential()
model.add(LSTM(7, return_sequences=True, input_dim=(look_back+6)))
#==============================================================================
...
testPredictPlot[len(trainPredict)+1:len(trainPredict)+1+len(testPredict), :] = testPredict
# plot baseline and predictions
plt.plot(dataset[look_back:,6])
